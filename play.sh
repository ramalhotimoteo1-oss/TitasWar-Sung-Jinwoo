...
while true; do
    ...
    printf "${WHITE}║${RESET}  TWM Multi-contas        %s  ${WHITE}║${RESET}\n" "$now"
    printf "${WHITE}╠%s╣${RESET}\n" "$LINE"
    ...
    while IFS='|' read -r srv user _enc <&3; do
        ...
        case "$status" in
            ...
            restarting) col="\033[33m"; label="restart" ;;
            starting) col="\033[33m"; label="start" ;;
            dead) col="\033[31m"; label="erro" ;;
            *) col="\033[33m"; label="$status" ;;
        esac

        printf "${WHITE}║${RESET} %b* [%-2s] %-16s %-8s ${WHITE}║${RESET}\n" \
        "$col" "$tag" "$user" "$label"
